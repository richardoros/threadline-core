"""Threadline SessionStart hook for Claude Code.

This script is registered as a Claude Code SessionStart hook.  Every time a
Claude Code session starts, Claude Code calls it with a JSON payload on stdin.
The script builds a Threadline event and POSTs it to the local Threadline API
so the session is opened in long-term memory before any work begins.

HOW IT IS CALLED
----------------
Claude Code invokes this script as a subprocess, piping JSON on stdin:

    python3 /path/to/threadline_session_start.py

stdin JSON shape (Claude Code SessionStart payload):
    {
        "session_id": "...",
        "cwd": "/absolute/path/to/project",
        "transcript_path": "/path/to/session.jsonl",  # path string, not contents
        "source": "startup"   # one of: startup, resume, clear, compact
    }

DESIGN DECISIONS
----------------
- STDLIB ONLY (json, os, sys, pathlib, urllib.request, re, subprocess) — this
  script must never require a pip install; it runs inside whatever Python
  Claude Code finds on the system.
...
- Privacy: only session_id, cwd-derived project key, the TRANSCRIPT PATH
  (a string — not the transcript contents), and repo/branch metadata are sent.
  No user content is ever read or transmitted.
...
# ---------------------------------------------------------------------------
# Project key helpers
# Duplicated from threadline_session_end.py to keep this hook standalone
# (stdlib-only, no shared import). Source of truth: threadline_session_end.py.
# ---------------------------------------------------------------------------
...
def build_event(payload: dict) -> dict:
    """Build an AgentEventIn-shaped dict from a Claude Code SessionStart payload.

    The top-level ``session_id`` is set to None — that field is reserved for
    Threadline's own internal AgentSession primary key, which does not exist
    yet when this event is sent.  The client's Claude Code session id is
    carried in ``details.client_session_id`` instead, where the ingest service
    can read it to create the AgentSession and store it as
    ``AgentSession.external_session_id``.

    Args:
        payload: The JSON object Claude Code sends on stdin for the SessionStart
                 hook.  Expected keys: session_id, cwd, transcript_path,
                 source (optional, one of: startup/resume/clear/compact).
    ...
    # Build the human-readable summary. The source suffix is omitted gracefully
    # when absent so the summary is always clean.
    summary = f"Claude Code session started in {cwd or 'unknown'}"
    if source:
    ...
    return {
        ...
        # Top-level session_id is None: Threadline's internal id does not exist
        # yet. The client id lives in details.client_session_id.
        "session_id": None,
        "summary": summary,
        "details": {
...
# ---------------------------------------------------------------------------
# HTTP transport (identical contract to threadline_session_end.py)
# ---------------------------------------------------------------------------
...
def post_event(event: dict, endpoint: str, token: str, timeout: float = 3.0) -> bool:
    ...
    runs synchronously while Claude Code is starting up.  A long timeout would
    stall the user's terminal for several seconds at the start of every session.
...
def main() -> int:
    """Read the Claude Code SessionStart hook payload from stdin, post to Threadline.

    EXIT CODE IS ALWAYS 0.
    ----------------------
    Claude Code interprets a non-zero hook exit code as an error and may
    display a warning to the user or abort the startup sequence.  Even if
